...
exports.StoredProcedureTester = void 0;
...
class StoredProcedureTester {
     ...
     */
    async testCoreProcedures(client, summary) {
        ...
        await this.testProcedure(client, summary, 'User Management', 'sp_create_user_from_cognito', async () => {
            const testData = {
                ...
                p_cognito_user_id: faker_1.faker.string.uuid(),
            ...
            const result = await client.query('CALL sp_create_user_from_cognito($1, $2, $3, $4, $5, $6, $7, NULL)', [
        ...
        await this.testProcedure(client, summary, 'User Management', 'sp_update_user_profile', async () => {
            ...
            const result = await client.query('CALL sp_update_user_profile($1, $2, $3, $4, $5, $6, $7)', Object.values(testData));
            return { testData, result: result.rows };
        });
        // FFC Management (4)
        await this.testProcedure(client, summary, 'FFC Management', 'sp_create_ffc', async () => {
            ...
            const result = await client.query('CALL sp_create_ffc($1, $2, $3, $4, NULL)', [testData.p_tenant_id, testData.p_owner_user_id, testData.p_name, testData.p_description]);
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'FFC Management', 'sp_add_persona_to_ffc', async () => {
            ...
            const result = await client.query('CALL sp_add_persona_to_ffc($1, $2, $3)', Object.values(testData));
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'FFC Management', 'sp_update_ffc_member_role', async () => {
            ...
            const result = await client.query('CALL sp_update_ffc_member_role($1, $2, $3)', Object.values(testData));
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'FFC Management', 'sp_remove_ffc_member', async () => {
            // Create a temp persona to remove
            const tempPersonaResult = await client.query(`INSERT INTO personas (tenant_id, user_id, first_name, last_name) 
         VALUES ($1, $2, $3, $4) RETURNING id`, [this.tenantId, this.testUserId, 'Temp', 'Persona']);
            const tempPersonaId = tempPersonaResult.rows[0].id;
            // Add to FFC
            await client.query('INSERT INTO ffc_personas (tenant_id, ffc_id, persona_id, ffc_role) VALUES ($1, $2, $3, $4)', [this.tenantId, this.testFfcId, tempPersonaId, 'beneficiary']);
            // Test removal
            const result = await client.query('CALL sp_remove_ffc_member($1, $2)', [this.testFfcId, tempPersonaId]);
        ...
        await this.testProcedure(client, summary, 'Asset Management', 'sp_create_asset', async () => {
            ...
            const result = await client.query('CALL sp_create_asset($1, $2, $3, $4, $5, $6, $7, NULL)', Object.values(testData));
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'Asset Management', 'sp_update_asset', async () => {
            ...
            const result = await client.query('CALL sp_update_asset($1, $2, $3, $4, $5)', Object.values(testData));
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'Asset Management', 'sp_delete_asset', async () => {
            // Get the category ID we created earlier
            const categoryResult = await client.query(`SELECT id FROM asset_categories WHERE name = $1`, ['Test Category']);
            const categoryId = categoryResult.rows[0].id;
            // Create a temp asset to delete
            const tempAssetResult = await client.query(`INSERT INTO assets (tenant_id, name, category_id, estimated_value) 
         VALUES ($1, $2, $3, $4) RETURNING id`, [this.tenantId, 'Temp Asset', categoryId, 1000]);
            const tempAssetId = tempAssetResult.rows[0].id;
            const result = await client.query('CALL sp_delete_asset($1)', [tempAssetId]);
            return { result: result.rows };
        });
        await this.testProcedure(client, summary, 'Asset Management', 'sp_transfer_asset_ownership', async () => {
            ...
            const newPersonaResult = await client.query(`INSERT INTO personas (tenant_id, user_id, first_name, last_name) 
         VALUES ($1, $2, $3, $4) RETURNING id`, [this.tenantId, this.testUserId, 'New', 'Owner']);
            ...
            const result = await client.query('CALL sp_transfer_asset_ownership($1, $2, $3, $4)', Object.values(testData));
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'Asset Management', 'sp_update_asset_value', async () => {
            ...
            const result = await client.query('CALL sp_update_asset_value($1, $2, $3, $4)', Object.values(testData));
            return { testData, result: result.rows };
        });
        ...
        await this.testProcedure(client, summary, 'Asset Management', 'sp_search_assets', async () => {
            ...
            const result = await client.query('SELECT * FROM sp_search_assets($1, $2, $3, $4, $5, $6)', Object.values(testData));
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'Asset Management', 'sp_assign_asset_to_persona', async () => {
            ...
            const result = await client.query('CALL sp_assign_asset_to_persona($1, $2, $3, $4)', Object.values(testData));
            return { testData, result: result.rows };
        });
        ...
        await this.testProcedure(client, summary, 'Contact Management', 'sp_add_email_to_persona', async () => {
            ...
            const result = await client.query('CALL sp_add_email_to_persona($1, $2, $3, $4, $5)', Object.values(testData));
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'Contact Management', 'sp_add_phone_to_persona', async () => {
            ...
            const result = await client.query('CALL sp_add_phone_to_persona($1, $2, $3, $4, $5)', Object.values(testData));
            return { testData, result: result.rows };
        });
        ...
        await this.testProcedure(client, summary, 'Invitation Management', 'sp_create_invitation', async () => {
            const testData = {
                ...
                p_invitee_email: faker_1.faker.internet.email(),
                p_invitee_phone: faker_1.faker.phone.number('+1##########'),
                p_invitee_first_name: faker_1.faker.person.firstName(),
                p_invitee_last_name: faker_1.faker.person.lastName(),
                p_role: 'beneficiary'
            };
            const result = await client.query('CALL sp_create_invitation($1, $2, $3, $4, $5, $6, $7, $8, NULL, NULL, NULL)', Object.values(testData));
            return { testData, result: result.rows };
        });
        // Audit & Compliance (4)
        await this.testProcedure(client, summary, 'Audit', 'sp_log_audit_event', async () => {
            ...
            const result = await client.query('CALL sp_log_audit_event($1, $2, $3, $4, $5, $6, $7)', Object.values(testData));
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'Audit', 'sp_create_audit_event', async () => {
            ...
            const result = await client.query('CALL sp_create_audit_event($1, $2, $3, $4, $5)', Object.values(testData));
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'Audit', 'sp_get_audit_trail', async () => {
            ...
            const result = await client.query('SELECT * FROM sp_get_audit_trail($1, $2, $3, $4, $5)', Object.values(testData));
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'Audit', 'sp_generate_compliance_report', async () => {
            ...
            const result = await client.query('SELECT * FROM sp_generate_compliance_report($1, $2, $3, $4)', Object.values(testData));
            return { testData, result: result.rows };
        });
        // Reporting (1)
        await this.testProcedure(client, summary, 'Reporting', 'sp_get_ffc_summary', async () => {
            const result = await client.query('SELECT * FROM sp_get_ffc_summary($1)', [this.testFfcId]);
            return { result: result.rows };
        });
        // Session Context (2)
        await this.testProcedure(client, summary, 'Session', 'sp_set_session_context', async () => {
            ...
            const result = await client.query('CALL sp_set_session_context($1, $2)', Object.values(testData));
            return { testData, result: result.rows };
        });
        ...
        await this.testProcedure(client, summary, 'Utility', 'update_updated_at_column', async () => {
            ...
            const result = await client.query('UPDATE users SET first_name = $1 WHERE id = $2 RETURNING updated_at', ['Updated Name', this.testUserId]);
            return { result: result.rows };
        });
    }
    /**
     ...
     */
    async testEventSourcingProcedures(client, summary) {
        ...
        await this.testProcedure(client, summary, 'Event Sourcing', 'sp_append_event', async () => {
            ...
            const result = await client.query('CALL sp_append_event($1, $2, $3, $4, $5, $6, $7, NULL)', Object.values(testData));
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'Event Sourcing', 'sp_replay_events', async () => {
            ...
            const result = await client.query('SELECT * FROM sp_replay_events($1, $2, $3)', Object.values(testData));
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'Event Sourcing', 'sp_create_snapshot', async () => {
            ...
            const result = await client.query('CALL sp_create_snapshot($1, $2, $3, $4)', Object.values(testData));
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'Event Sourcing', 'sp_rebuild_projection', async () => {
            ...
            const result = await client.query('CALL sp_rebuild_projection($1, $2, $3)', Object.values(testData));
            return { testData, result: result.rows };
        });
    }
    /**
     ...
     */
    async testIntegrationProcedures(client, summary) {
        ...
        await this.testProcedure(client, summary, 'PII Management', 'sp_detect_pii', async () => {
            const testData = {
                ...
                p_entity_id: faker_1.faker.string.uuid()
            };
            const result = await client.query('SELECT * FROM sp_detect_pii($1, $2, $3, $4)', Object.values(testData));
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'PII Management', 'sp_update_pii_job_status', async () => {
            ...
            const jobResult = await client.query(`INSERT INTO pii_processing_jobs (tenant_id, job_type, status, processing_options) 
         VALUES ($1, $2, $3, $4) RETURNING id`, [this.tenantId, 'scan', 'pending', JSON.stringify({ entity_type: 'document', entity_id: faker_1.faker.string.uuid() })]);
            ...
            const result = await client.query('CALL sp_update_pii_job_status($1, $2, $3)', Object.values(testData));
            return { testData, result: result.rows };
        });
        ...
        await this.testProcedure(client, summary, 'Quillt Integration', 'sp_configure_quillt_integration', async () => {
            const testData = {
                p_tenant_id: this.tenantId,
                p_user_id: this.testUserId,
                p_connection_id: faker_1.faker.string.uuid(),
                p_configuration: JSON.stringify({ api_key: 'test-key' })
            };
            const result = await client.query('CALL sp_configure_quillt_integration($1, $2, $3, $4)', Object.values(testData));
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'Quillt Integration', 'sp_sync_quillt_data', async () => {
            ...
            const integrationResult = await client.query(`INSERT INTO quillt_integrations (tenant_id, user_id, quillt_connection_id, connection_status) 
         VALUES ($1, $2, $3, $4) RETURNING id`, [this.tenantId, this.testUserId, faker_1.faker.string.uuid(), 'connected']);
            const integrationId = integrationResult.rows[0].id;
            const testData = {
                p_integration_id: integrationId,
                p_sync_type: 'accounts'
            };
            const result = await client.query('CALL sp_sync_quillt_data($1, $2)', Object.values(testData));
            return { testData, result: result.rows };
        });
        ...
        await this.testProcedure(client, summary, 'Quillt Integration', 'sp_get_quillt_sync_status', async () => {
            const result = await client.query('SELECT * FROM sp_get_quillt_sync_status($1)', [this.testUserId]);
            return { result: result.rows };
        });
        // Real Estate (2)
        await this.testProcedure(client, summary, 'Real Estate', 'sp_sync_real_estate_data', async () => {
            ...
            const integrationResult = await client.query(`INSERT INTO real_estate_provider_integrations (tenant_id, provider_name, api_endpoint, is_active) 
         VALUES ($1, $2, $3, $4) 
        ...
        await this.testProcedure(client, summary, 'Real Estate', 'sp_get_real_estate_sync_history', async () => {
            ...
            const result = await client.query('SELECT * FROM sp_get_real_estate_sync_history($1, $2)', Object.values(testData));
            return { testData, result: result.rows };
        });
        // Advisor Companies (2)
        await this.testProcedure(client, summary, 'Advisor Companies', 'sp_manage_advisor_company', async () => {
            const testData = {
                ...
                p_contact_info: JSON.stringify({ phone: faker_1.faker.phone.number() }),
                p_company_id: null
            };
            const result = await client.query('CALL sp_manage_advisor_company($1, $2, $3, $4, $5, $6, NULL)', Object.values(testData));
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'Advisor Companies', 'sp_get_advisor_companies', async () => {
            ...
            const result = await client.query('SELECT * FROM sp_get_advisor_companies($1, $2, $3)', Object.values(testData));
            return { testData, result: result.rows };
        });
        ...
        await this.testProcedure(client, summary, 'Integration Health', 'sp_check_integration_health', async () => {
            ...
            const result = await client.query('SELECT * FROM sp_check_integration_health($1, $2)', Object.values(testData));
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'Integration Health', 'sp_retry_failed_integration', async () => {
            ...
            const result = await client.query('CALL sp_retry_failed_integration($1, $2, $3)', Object.values(testData));
            return { testData, result: result.rows };
        });
        // Builder.io (3)
        await this.testProcedure(client, summary, 'Builder.io', 'sp_configure_builder_io', async () => {
            const testData = {
                p_tenant_id: this.tenantId,
                p_api_key: faker_1.faker.string.alphanumeric(32),
                p_space_id: faker_1.faker.string.uuid(),
                p_environment: 'production'
            };
            const result = await client.query('CALL sp_configure_builder_io($1, $2, $3, $4)', Object.values(testData));
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'Builder.io', 'sp_refresh_builder_content', async () => {
            ...
            const integrationResult = await client.query(`INSERT INTO builder_io_integrations (tenant_id, api_key, space_id, environment, is_active) 
         VALUES ($1, $2, $3, $4, $5) 
         ON CONFLICT (tenant_id) 
         DO UPDATE SET api_key = EXCLUDED.api_key, updated_at = NOW()
         RETURNING id`, [this.tenantId, 'test-key', faker_1.faker.string.uuid(), 'production', true]);
            const integrationId = integrationResult.rows[0].id;
            const testData = {
                p_integration_id: integrationId,
                p_content_type: 'page'
            };
            const result = await client.query('CALL sp_refresh_builder_content($1, $2)', Object.values(testData));
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'Builder.io', 'sp_get_builder_content_status', async () => {
            const result = await client.query('SELECT * FROM sp_get_builder_content_status($1)', [this.tenantId]);
            return { result: result.rows };
        });
        // Translation (2)
        await this.testProcedure(client, summary, 'Translation', 'sp_manage_translation', async () => {
            ...
            const result = await client.query('CALL sp_manage_translation($1, $2, $3, $4, $5, $6, $7, $8, NULL)', Object.values(testData));
            return { testData, result: result.rows };
        });
        await this.testProcedure(client, summary, 'Translation', 'sp_get_translations', async () => {
            ...
            const result = await client.query('SELECT * FROM sp_get_translations($1, $2, $3)', Object.values(testData));
            return { testData, result: result.rows };
        });
        ...
        await this.testProcedure(client, summary, 'System', 'sp_update_system_configuration', async () => {
            ...
            const result = await client.query('CALL sp_update_system_configuration($1, $2, $3, $4)', Object.values(testData));
            return { testData, result: result.rows };
        });
    }
    /**
    ...
    async setupTestData(client) {
        ...
        const tenantResult = await client.query(`INSERT INTO tenants (id, name, display_name) 
       VALUES ($1, $2, $3) 
       ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name 
       RETURNING id`, [this.tenantId, 'test-tenant', 'Test Tenant']);
        // Create test user
        const userResult = await client.query(`INSERT INTO users (tenant_id, cognito_user_id, first_name, last_name, status) 
       VALUES ($1, $2, $3, $4, $5) 
       RETURNING id`, [this.tenantId, faker_1.faker.string.uuid(), 'Test', 'User', 'active']);
       ...
       VALUES ($1, $2, $3, $4) 
       RETURNING id`, [this.tenantId, this.testUserId, 'Test', 'Persona']);
        this.testPersonaId = personaResult.rows[0].id;
        // Create test FFC
        const ffcResult = await client.query(`INSERT INTO fwd_family_circles (tenant_id, owner_user_id, name, description) 
       VALUES ($1, $2, $3, $4) 
       RETURNING id`, [this.tenantId, this.testUserId, 'Test FFC', 'Test Family Circle']);
        this.testFfcId = ffcResult.rows[0].id;
        // Add persona to FFC
        await client.query(`INSERT INTO ffc_personas (tenant_id, ffc_id, persona_id, ffc_role) 
       VALUES ($1, $2, $3, $4)`, [this.tenantId, this.testFfcId, this.testPersonaId, 'owner']);
        // Create test asset category first (global table, no tenant_id)
        const categoryResult = await client.query(`INSERT INTO asset_categories (name, code, description, sort_order) 
       VALUES ($1, $2, $3, $4) 
...
exports.StoredProcedureTester = StoredProcedureTester;

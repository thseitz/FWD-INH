...
exports.SafeProcedureTester = void 0;
const faker_1 = require("@faker-js/faker");
class SafeProcedureTester {
    pool;
    ...
    async setupPersistentTestData() {
        ...
        try {
         ...
         RETURNING id`, [
            ...
            let personaResult = await client.query(`SELECT id FROM personas 
         WHERE user_id = $1 AND first_name = $2 AND last_name = $3`, [this.testUserId, 'Test', 'Persona']);
            if (personaResult.rows.length === 0) {
                personaResult = await client.query(`INSERT INTO personas (tenant_id, user_id, first_name, last_name) 
           VALUES ($1, $2, $3, $4) 
           RETURNING id`, [
                    this.tenantId,
                    this.testUserId,
            ...
            let ffcResult = await client.query(`SELECT id FROM fwd_family_circles WHERE name = 'Test FFC Persistent' LIMIT 1`);
            if (ffcResult.rows.length === 0) {
                ffcResult = await client.query(`INSERT INTO fwd_family_circles (tenant_id, name, description, owner_user_id, status) 
           ...
           RETURNING id`, [
            ...
            await client.query(`INSERT INTO ffc_personas (tenant_id, ffc_id, persona_id, ffc_role, joined_at)
         VALUES ($1, $2, $3, 'owner', NOW())
         ON CONFLICT (ffc_id, persona_id) DO NOTHING`, [this.tenantId, this.testFfcId, this.testPersonaId]);
           ...
           RETURNING id`);
            ...
            let assetResult = await client.query(`SELECT id FROM assets WHERE name = 'Test Asset Persistent' LIMIT 1`);
            if (assetResult.rows.length === 0) {
                assetResult = await client.query(`INSERT INTO assets (tenant_id, category_id, name, description, status) 
           VALUES ($1, $2, $3, $4, 'active') 
           RETURNING id`, [
                    this.tenantId,
                    this.testCategoryId,
     ...
     * Cleanup persistent test data
     */
    async cleanupPersistentTestData() {
     ...
     */
    async testRLSHelperFunctions(summary) {
     ...
     */
    async testUserManagement(summary) {
        ...
        await this.testFunctionSafe(summary, 'User Management', 'sp_create_user_from_cognito', async (client) => {
            const testData = {
                ...
                p_phone: faker_1.faker.string.numeric('##########'),
     ...
     */
    async testFFCManagement(summary) {
        ...
        await this.skipFunction(summary, 'FFC Management', 'sp_update_ffc_member_role', 'Skipped due to audit_action_enum mismatch');
     ...
     */
    async testAssetManagement(summary) {
        ...
        await this.testFunctionSafe(summary, 'Asset Management', 'sp_update_asset', async (client) => {
            const testData = {
                ...
                p_description: faker_1.faker.lorem.sentence(),
                p_estimated_value: faker_1.faker.number.float({ min: 10000, max: 1000000, fractionDigits: 2 }),
                p_status: 'active',
                p_metadata: { updated: true },
                p_updated_by: this.testUserId
            };
            const result = await client.query('SELECT sp_update_asset($1, $2, $3, $4, $5::status_enum, $6::jsonb, $7) as success', Object.values(testData));
            return { testData, result: result.rows[0] };
        });
     ...
     */
    async testContactManagement(summary) {
     ...
     */
    async testAuditCompliance(summary) {
        console.log('📊 Testing Audit & Compliance Functions...\n');
        // Skip functions that require proper audit_action_enum values
        await this.skipFunction(summary, 'Audit & Compliance', 'sp_log_audit_event', 'Requires valid audit_action_enum');
     ...
     */
    async testEventSourcing(summary) {
     ...
     */
    async testIntegrations(summary) {
     ...
     */
    async testSubscriptionPayment(summary) {
     ...
     */
    async testFunctionSafe(summary, category, procedureName, testFunc) {
     ...
     */
    async skipFunction(summary, category, procedureName, reason) {
        const result = {
            procedureName,
            category,
            status: 'skipped',
...
exports.SafeProcedureTester = SafeProcedureTester;
